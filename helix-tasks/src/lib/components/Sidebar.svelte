<script lang="ts">
  import { filters, projects, categories, tasks, currentView, appMode } from '$lib/store';
...
<aside class="sidebar">
  ...
  <div class="mode-toggle">
    <button class={$appMode === 'tasks' ? 'active' : ''} on:click={() => ($appMode = 'tasks')}>
      ✅ Tasks
    </button>
    <button class={$appMode === 'notes' ? 'active' : ''} on:click={() => ($appMode = 'notes')}>
      📝 Notes
    </button>
  </div>
...
<style>
  ...
  .nav-icon { font-size: 15px; line-height: 1; width: 16px; text-align: center; }
