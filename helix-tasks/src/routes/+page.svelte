<script lang="ts">
  ...
  async function openFolder() {
    try {
      ...
      if (selected && typeof selected === 'string') {
        ...
        await loadTasks();
        // Also refresh notes if in notes mode
        if ($appMode === 'notes') await loadNotes();
      }
    } catch (e) {
  ...
  async function loadNotes() {
    if (!$folderPath) return;
    try {
      const result = await invoke<any[]>('list_notes', { folderPath: $folderPath });
      $notes = result;
    } catch (e) {
      error = String(e);
    }
  ...
  async function handleQuickAdd(e: CustomEvent<Partial<Task>>) {
    if (!$folderPath) {
      error = 'Please open a folder first — click the folder button in the sidebar.';
      return;
    }
  ...
  async function handleSave(e: CustomEvent<Task>) {
    const task = e.detail;
    if (!$folderPath) {
      error = 'Please open a folder first.';
      return;
    }
    try {
      if (!task.filePath) {
    ...
    <div class="toolbar">
      <div class="toolbar-left">
        {#if $folderPath}
          ...
          {:else}
            <span class="task-count">{$notes.length} notes</span>
          {/if}
        {:else}
          <span class="no-folder">No folder open — click "Open Folder" in the sidebar</span>
        {/if}
      </div>
      <div class="toolbar-right">
        {#if $folderPath && $appMode === 'tasks'}
          ...
          <button class="new-task-btn" on:click={newTask}>+ New Task</button>
        {/if}
      </div>
    </div>
    ...
    {#if !$folderPath}
      ...
      <div class="welcome">
        <div class="welcome-card">
          ...
          <h1>Helix Tasks</h1>
          <p>Markdown-native task manager and note editor.<br/>Each task is a <code>.md</code> file with YAML frontmatter.</p>
          ...
          <p class="welcome-hint">Works with any folder of <code>.md</code> files — including your Obsidian vault.</p>
        </div>
      </div>
...
<style>
  ...
  .dismiss { margin-left: auto; background: none; border: none; color: var(--red); cursor: pointer; font-size: 14px; }
  ...
  .welcome-card p { color: var(--muted); font-size: 15px; line-height: 1.65; }
  .welcome-card code { background: rgba(255,255,255,0.07); border-radius: 4px; padding: 1px 6px; font-size: 13px; color: var(--teal); }
  ...
  .welcome-hint { font-size: 13px !important; }

  /* Loading */
  .loading { flex: 1; display: flex; align-items: center; justify-content: center; gap: 12px; color: var(--muted); font-size: 14px; }
